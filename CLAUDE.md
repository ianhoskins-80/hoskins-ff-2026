...
- There are **two** separate ESPN APIs in play, don't conflate them: the fantasy API (`lm-api-reads.fantasy.espn.com`) for league/roster/score data, and the public NFL scoreboard (`site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard`) for real live-game status (`status.type.state`). The fantasy API's per-player stats only say whether ESPN has posted *any* stat this week, not whether the game is still in progress — that's what the scoreboard API is for (see Live Scoring in README.md).
