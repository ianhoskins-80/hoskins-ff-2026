...
- Re-fetches every 5 minutes while the tab stays open, aligned to the backend's refresh cycle (`scheduleAlignedRefresh()`: waits out whatever's left until the next `:00/:05/:10...` mark, then a plain `setInterval` from there). That `setInterval` is a background-tab blind spot, though — browsers throttle or fully suspend it while the tab is hidden, with no built-in way to catch up once it resumes, so a tab left backgrounded across a refresh mark can sit on stale data well past when it should've updated. A `visibilitychange` listener (the same one that already pauses/resumes the viewer-count heartbeat) fires an immediate extra fetch the moment the tab becomes visible again, so reopening a stale background tab is never worse than a few seconds behind, not however long it sat unwatched.
...
- **Shared players across leagues:** the two leagues are drafted independently, so the same real NFL player can be rostered by a team in each at once — same real stat line, so always the same points. `groupLiveScoringByPlayer()` groups the board by `playerId` before display, so a shared player gets one row naming every team that has them (`Team A, Team B`, each name in its own `teamLineColorForTeam()` shade) instead of two duplicate-looking rows. With the team filter active, a shared player's row still shows if *any* of their teams is selected, but only the selected team(s) are named on that row — not one you didn't ask to isolate.
