...
Below "This Week's Matchups," a matrix table: one row per team (combined across leagues, sorted alphabetically by team name), one column per real position (QB/RB/WR/TE/D-ST/K/HC — a FLEX-WR counts under WR, not a separate FLEX column). Each cell sums each contributing starter's **actual** points where ESPN has posted them, falling back to their **projected** points individually for anyone who hasn't played yet — so a cell is only ever fully real (solid) once *every* starter at that position has an actual stat; if even one is still projected, the whole cell stays italic (`matrix-projected`), since the total isn't finalized. Hovering (or the click-through modal) breaks down which players contributed a real stat vs. a projection. `allActual` (per position bucket) tracks this — not to be confused with the now-removed `allProjected`, which only flipped false once *any* player had posted a stat, letting a cell that was mostly still-projected render as if it were a finished, fully-real number. Built entirely from the same per-game roster data already used by the roster lightbox.
